...
These user stories are fewer and accommodate holiday interruptions during the project timeframe, and still focus on the core learning goals of the project.
...
I see a "delete" button next to some merchants
Merchants who have 'fulfilled' items for any order cannot be deleted
```
...
And I click on a "delete" button for a merchant
I am returned to the merchant index page
And I see a flash message that the merchant's account is now deleted
That merchant's items are no longer for sale.
```
